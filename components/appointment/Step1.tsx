  ...
  return (
    ...
    <div className="relative mb-6">
      ...
      <input
        ...
        placeholder=" "
      />
      {errors.step1?.name && <p className="text-red-500 text-sm">{errors.step1.name?.message}</p>}
    </div>
  
    <div className="grid gap-4 md:gap-6 md:grid-cols-2"> {/* Use grid layout for larger screens */}
      <div className="relative mb-6">
        <label htmlFor="country" className="text-gray-500 text-sm md:text-base">Country:</label>
        <select
        ...
        {errors.step1?.country && <p className="text-red-500 text-sm">{errors.step1.country.message}</p>}
      </div>
  
      <div className="relative mb-6">
        <label htmlFor="phone" className="text-gray-500 text-sm md:text-base">Phone number:</label>
        <input
          type="tel"
          ...
          placeholder=" "
        />
        {errors.step1?.phone && <p className="text-red-500 text-sm">{errors.step1.phone.message}</p>}
      </div>
    </div>
  
    <div className="grid gap-4 md:gap-6 md:grid-cols-2"> {/* Second grid for other fields */}
      <div className="relative mb-6">
        <label htmlFor="date" className="text-gray-500 text-sm md:text-base">Date:</label>
        <input
          type="date"
          {...register('step1.date')}
          id="date"
          className="block w-full px-4 py-2 md:py-3 text-sm md:text-base text-gray-900 bg-gray-100 rounded-md focus:ring-blue-500 focus:outline-none"
        />
        {errors.step1?.date && <p className="text-red-500 text-sm">{errors.step1.date.message}</p>}
      </div>
  
      <div className="relative mb-6">
        <label htmlFor="time" className="text-gray-500 text-sm md:text-base">Time:</label>
        <select
          {...register('step1.time')}
          id="time"
          className="block w-full px-4 py-2 md:py-3 text-sm md:text-base text-gray-900 bg-gray-100 rounded-md focus:ring-blue-500 focus:outline-none"
        ...
        {errors.step1?.time && <p className="text-red-500 text-sm">{errors.step1.time.message}</p>}
      </div>
  
      <div className="relative mb-6">
        <label htmlFor="duration" className="text-gray-500 text-sm md:text-base">Duration:</label>
        <select
        ...
        {errors.step1?.duration && <p className="text-red-500 text-sm">{errors.step1.duration.message}</p>}
      </div>
    </div>
  
    {selectedTime && selectedDuration && (
      <div className="relative mb-6">
        <label htmlFor="endTime" className="text-gray-500 text-sm md:text-base">End Time:</label>
        <input
          type="text"
          id="endTime"
